...
function validateForm() {
  var CCN = trimBetweenSpaces(
    trimBegEndSpaces(stripOffNonDigit(document.ccform.UMcard.value))
  );
  var expireDate = trimBetweenSpaces(
    trimBegEndSpaces(
      stripOffNonDigit(
        document.ccform.UMexpirM.value + document.ccform.UMexpirY.value
      )
    )
  );
  var bankRouting = trimBetweenSpaces(
    trimBegEndSpaces(document.ccform.UMrouting.value)
  );
  var bankAccount = trimBetweenSpaces(
    trimBegEndSpaces(document.ccform.UMaccount.value)
  );
  var SSN = trimBetweenSpaces(
    trimBegEndSpaces(stripOffNonDigit(document.ccform.UMssn.value))
  );
  var dlNum = trimBetweenSpaces(
    trimBegEndSpaces(document.ccform.UMdlnum.value)
  );
  var dlState = trimBetweenSpaces(
    trimBegEndSpaces(document.ccform.UMdlstate.value)
  );
  var amount = trimBetweenSpaces(
    trimBegEndSpaces(stripOffNonDigit(document.ccform.UMamount.value))
  );
  var invoice = trimBetweenSpaces(
    trimBegEndSpaces(document.ccform.UMinvoice.value)
  );
  var name = document.ccform.UMname.value;
  var street = document.ccform.UMstreet.value;
  var zip = trimBetweenSpaces(
    trimBegEndSpaces(stripOffNonDigit(document.ccform.UMzip.value))
  );

  if (CCN.length == 0 || expireDate.Length == 0) {
    if (
      bankRouting.length == 0 ||
...
function isAlphaSymbols(objValue, symbols) {
  ...
  for (var i = 0; i < objValue.length; i++) {
    ...
    if (isAlphaChar(ch) == false) {
      if (symbols.indexOf(ch) < 0) return false;
    }
  }
  return true;
}
...
function trimBegEndSpaces(object_value) {
  ...
  while (
    leading_blanks <= string_end &&
    object_value.charAt(leading_blanks) == " "
  ) {
    leading_blanks++;
  }
  ...
  while (
    string_end > leading_blanks &&
    object_value.charAt(string_end) == " "
  ) {
    string_end--;
  }

  return (object_value = object_value.substring(
    leading_blanks,
    string_end + 1
  ));
}
...
function trimBetweenSpaces(objValue) {
  ...
  if (newValue == null) return objValue;
